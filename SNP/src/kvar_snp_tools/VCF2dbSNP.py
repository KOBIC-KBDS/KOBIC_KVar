...
VCF_INFO_ID_RENAMES = {
    '1kGP_AF': 'AF1KG',
    'GP1K_AF': 'AF1KG',
    '1kGP_EAS_AF': 'AF1KG_EAS',
    'GP1K_EAS_AF': 'AF1KG_EAS',
    '61KJPN': 'JPN61K',
    '61KJPN_AC': 'JPN61K_AC',
    '61KJPN_AF': 'JPN61K_AF',
    '61KJPN_AN': 'JPN61K_AN',
...
class VCF2dbSNPConverter:
    ...
    def __init__(self, error_handler: Optional[ErrorHandler] = None):
        ...
        self._retain_parsed_rows = True
        self._parsed_row_count = 0
        self._observed_output_info_ids = set()
        self._row_spool: Optional[TextIO] = None
    ...
    def convert_vcf_to_dbsnp(
        ...
        row_spool = None
        try:
            output_dir = os.path.dirname(os.path.abspath(output_file_path)) or None
            row_spool = self._parse_vcf_to_spool(vcf_file_path, output_dir=output_dir)
        except Exception:
            print("Conversion stopped due to VCF parse failure.")
        ...
        try:
            ...
            if error_report_path is None:
                error_report_path = default_error_report_path(output_file_path)
            ...
            try:
                self._write_dbsnp_vcf(output_file_path, row_spool=row_spool)
            except Exception as e:
    ...
    def _parse_vcf_to_spool(self, vcf_file_path: str, output_dir: Optional[str] = None) -> TextIO:
        """Parse once and spool output rows without retaining row objects."""
        row_spool = tempfile.SpooledTemporaryFile(
            max_size=64 * 1024 * 1024,
            mode='w+t',
            encoding='utf-8',
            dir=output_dir,
        )
        self._row_spool = row_spool
        try:
            self._parse_vcf_file(vcf_file_path, retain_rows=False)
            row_spool.flush()
            row_spool.seek(0)
            return row_spool
        except Exception:
            ...
            self._row_spool = None

    def _parse_vcf_file(self, vcf_file_path: str, retain_rows: bool = True) -> None:
        """Parse a VCF file with gzip support."""
        self.parser._seen_local_ids.clear()
        self.parser._seen_variant_sites.clear()
        self.parser.data_rows.clear()
        self._retain_parsed_rows = retain_rows
        self._parsed_row_count = 0
        self._observed_output_info_ids.clear()
        ...
        print(f"Parsing complete: {len(header_lines)} header lines, {self._parsed_row_count} variants")
    ...
    def _parse_header(self, header_lines: List[str]) -> None:
        ...
        self._validate_required_header()

    def _validate_required_header(self) -> None:
        """Validate required VCF header fields for the single-pass path."""
        if not self.parser.header.metadata.fileformat:
            self.error_handler.create_error(
                ErrorCode.MISSING_FILEFORMAT,
                line_number=1
            )
        elif not self.parser.header.metadata.fileformat.startswith('VCFv'):
            self.error_handler.create_error(
                ErrorCode.INVALID_FILEFORMAT,
                line_number=1,
                actual_value=self.parser.header.metadata.fileformat,
                expected_value="VCFv format (e.g. VCFv4.1, VCFv4.2, VCFv4.3, VCFv4.4, VCFv4.5)"
            )

        if not self.parser.header.column_header:
            self.error_handler.create_error(
    ...
    def _parse_info_tag_definition(self, line: str, line_number: int) -> None:
        ...
        if match:
            ...
            description = self._unescape_vcf_description(description)
    ...
    def _parse_column_header(self, line: str, line_number: int) -> None:
        ...
        if len(columns) < len(required_columns) or columns[:len(required_columns)] != required_columns:
            self.error_handler.create_error(
    ...
    def _parse_data(self, data_lines: List[Tuple[int, str]]) -> None:
        ...
        for line_number, line in data_lines:
            ...
            if row:
                ...
                self._observed_output_info_ids.update(
                    get_vcf_info_output_id(tag_id)
                    for tag_id in row.info
                    if is_dbsnp_output_info_id(tag_id)
                )
                if self._row_spool is not None:
                    self._write_data_row(self._row_spool, row)
                elif self._retain_parsed_rows:
    ...
    def _write_dbsnp_vcf(self, output_file_path: str, row_spool: Optional[TextIO] = None) -> None:
        ...
        with output_opener(output_file_path, 'wt', encoding='utf-8') as f:
            ...
            if row_spool is not None:
                row_spool.seek(0)
                shutil.copyfileobj(row_spool, f)
            else:
    ...
    def _write_metadata(self, f) -> None:
        """Write metadata."""
        # dbSNP submission VCF output is standardized to VCF v4.1.
    ...
    def _write_info_tag_definitions(self, f) -> None:
        ...
        for tag_id, tag_def in self.parser.header.info_tags.items():
            ...
            escaped_description = self._escape_vcf_description(tag_def.description)
            f.write(f'##INFO=<ID={output_info_id},Number={tag_def.number},Type={tag_def.type},Description="{escaped_description}">\n')
        ...
        for output_info_id in DBSNP_OUTPUT_INFO_ORDER:
            ...
            written_info_ids.add(output_info_id)
            escaped_description = self._escape_vcf_description(description)
            f.write(f'##INFO=<ID={output_info_id},Number={number},Type={type_str},Description="{escaped_description}">\n')

    def _used_output_info_ids(self) -> set:
        """Return retained INFO IDs that are present in parsed rows."""
        used_info_ids = set(self._observed_output_info_ids)
        for row in self.parser.data_rows:
            for tag_id in row.info:
    ...
    def _write_format_tag_definitions(self, f) -> None:
        ...
        standard_formats = {
            'NA': ('1', 'Integer', 'Number of alleles for the population'),
            'NS': ('1', 'Integer', 'Number of samples for the population'),
            'FRQ': ('.', 'Float', 'Frequency of each alternate allele'),
            'AC': ('.', 'Integer', 'Allele count for each alternate allele'),
        }

        for format_id, default_definition in standard_formats.items():
            tag_def = self.parser.header.format_tags.get(format_id)
            if tag_def:
                number, type_str, description = tag_def.number, tag_def.type, tag_def.description
            else:
                number, type_str, description = default_definition
            escaped_description = self._escape_vcf_description(description)
            f.write(f'##FORMAT=<ID={format_id},Number={number},Type={type_str},Description="{escaped_description}">\n')

        # Existing FORMAT tags.
        for tag_id, tag_def in self.parser.header.format_tags.items():
            if tag_id not in standard_formats:
                escaped_description = self._escape_vcf_description(tag_def.description)
                f.write(f'##FORMAT=<ID={tag_def.id},Number={tag_def.number},Type={tag_def.type},Description="{escaped_description}">\n')
